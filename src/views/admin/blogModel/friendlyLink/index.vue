    ...
    <el-row :gutter="10">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-has-permi="['blog:friendlyLink:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-has-permi="['blog:friendlyLink:update']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-has-permi="['blog:friendlyLink:delete']"
        >删除</el-button>
      </el-col>
    </el-row>
...
<script>
import { getFriendlyLink, getFriendlyLinkById, addFriendlyLink, updateFriendlyLink, delFriendlyLink } from '@/api/admin/blog/friendlyLink'
...
export default {
  ...
  methods: {
    getList () {
      getFriendlyLink(this.queryParams).then(res => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.id !== undefined) {
            updateFriendlyLink(this.form).then(res => {
              this.$httpResponse(res.message)
              this.open = false
              this.getList()
            })
          } else {
            addFriendlyLink(this.form).then(res => {
              this.$httpResponse(res.message)
              this.open = false
              this.getList()
            })
